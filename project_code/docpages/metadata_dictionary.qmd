...
| event_date_precision | Text | Certainty level of `event_date`. Values: `exact` (original or format-corrected), `month` (day filled with `01`), `month_inferred` (month borrowed from a neighbouring record), `year_inferred` (year borrowed from a neighbouring record), `day_adjusted` (nonexistent date corrected to nearest valid day), `estimated` (torn record, day averaged from a range) |
...
| baptized_birth_date_precision | Text | Certainty level of `baptized_birth_date`. Includes all values from `event_date_precision`, plus `inferred_from_age` (birth date back-calculated by subtracting a recorded age expression from the event date) |
...
| event_date_precision | Text | Certainty level of `event_date`. Values: `exact`, `month`, `month_inferred`, `year_inferred`, `day_adjusted`, `estimated` (see Baptisms for definitions) |
...
| husband_birth_date_precision | Text | Certainty level of `husband_birth_date`. Includes all values from `event_date_precision`, plus `inferred_from_age` |
...
| wife_birth_date_precision | Text | Certainty level of `wife_birth_date`. Includes all values from `event_date_precision`, plus `inferred_from_age` |
...
| deceased_birth_date_precision | Text | Certainty level of `deceased_birth_date`. Includes all values from `event_date_precision`, plus `inferred_from_age` |
...
```{r}
places <- readr::read_csv("../../data/clean/places.csv", show_col_types = FALSE)
places %>% glimpse()
```
:::
...
| birth_date_precision | Text | Certainty level of `birth_date`. Includes all values from `event_date_precision`, plus `inferred_from_age` |
| birth_place | Text       | Place of birth |
| death_date | Date          | Recorded or inferred date of death in ISO 8601 format |
| death_date_precision | Text | Certainty level of `death_date`. Values: `exact`, `month`, `month_inferred`, `year_inferred`, `day_adjusted`, `estimated` (see Baptisms for definitions) |
